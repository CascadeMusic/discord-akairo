...
class AkairoHandler extends EventEmitter {
  ...
  static findExport(obj) {
    if (module.__esModule) {
      const _default = Reflect.get(obj, "default");
      if (Util.isClass(_default)) {
        return _default;
      }

      let _class = null;
      for (const prop of Object.values(obj)) {
        if (Util.isClass(prop)) {
          _class = prop;
          break;
        }
      }
    ...
    return Util.isClass(obj) ? obj : null;
  }
  ...
  load(thing, isReload = false) {
    ...
    let mod = AkairoHandler.findExport(thing)
