...
class Argument {
  ...
  async process(message, phrase) {
    ...
    if (!phrase && optional) {
      if (this.otherwise != null) {
        return await doOtherwise(null);
      }

      return await intoCallable(this.default)(message, { phrase, failure: null });
    }

    const res = await this.cast(message, phrase);
    if (Argument.isFailure(res)) {
      if (this.otherwise != null) {
        return await doOtherwise(res);
      }
      ...
      return this.default == null
        ? res
        : await intoCallable(this.default)(message, { phrase, failure: res });
    }
  ...
  static validate(type, predicate) {
    return async (message, phrase) => {
      ...
      return await predicate.call(this, message, phrase, res)
        ? res
        : null;
    };
  }
